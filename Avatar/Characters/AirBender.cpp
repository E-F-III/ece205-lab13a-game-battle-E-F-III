...
AirBender::AirBender(string& characterName, int& raceCode) : PlayerCharacter(characterName, raceCode) {
    airBendingSystem = new AirBending(); // Initialize the air bending system pointer
}
...
void AirBender::performAction() {
    ...
    switch (choice) {
        case 0:
            ...
            airBendingSystem->airSlice(*this); // Passing *this reference to modify health
            break;
        case 1:
            cout << "\n--- Action Selected: Air Shield ---\n" << endl;
            airBendingSystem->airShield(*this);  // Passing *this reference to modify health
            break;
        case 2:
            cout << "\n--- Action Selected: Flight ---\n" << endl;
            airBendingSystem->flight(*this);      // Passing *this reference for effect logic
            break;
    }
}
...
void AirBender::printStats() {
    PlayerCharacter::printStats(); // Call parent function first
    cout << "Profession: AirBender" << endl;
    cout << "------------------------------------" << endl;
}


/// AirBender-specific greeting (This is a simple method, no logic needed here)
